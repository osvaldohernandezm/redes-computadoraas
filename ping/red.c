#include "red.h"
int i;
void parametros(int ari,char **arc,int *ttl,int *pac,unsigned char *tuip,char *mensaje)//./a.out wlan0 -i 192.168.1.15 -t 64 -p 5 -h www.google.com
{
        int yapuso=0;
        for(i=0;i<ari;i++)
        {
                if(!strcmp(arc[i],"-i")&&!yapuso)
                {
                        sscanf(arc[i+1],"%d.%d.%d.%d",(int*)&tuip[0],(int*)&tuip[1],(int*)&tuip[2],(int*)&tuip[3]);
                        yapuso=1;
                }
                if(!strcmp(arc[i],"-h")&&!yapuso)
                {
                        ...
                        {
                                fscanf(host," %d.%d.%d.%d",(int*)&tuip[0],(int*)&tuip[1],(int*)&tuip[2],(int*)&tuip[3]);
                        }
                        else
                ...
                if(!strcmp(arc[i],"-t"))
                {
                        sscanf(arc[i+1],"%d",ttl);
                }
                if(!strcmp(arc[i],"-p"))
                {
                        sscanf(arc[i+1],"%d",pac);
                }
		if(!strcmp(arc[i],"-m"))
		{
			strcpy(mensaje,arc[i+1]);
		}
         }
