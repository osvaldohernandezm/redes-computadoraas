#include "red.h"
int i;
void parametros(int ari,char **arc,int *r1,int *r2,char **miarg)//./a.out wlan0 -i 192.168.1.15
{
        for(i=0;i<ari;i++)
        {
		if(!memcmp(arc[i],"-r",2))
		{
			sscanf(arc[i+1],"%d-%d",r1,r2);
		}
		if(!memcmp(arc[i],"-s",2))
		{
			memcpy(miarg[0]+0,arc[i+1],50);
		}
		if(!memcmp(arc[i],"-u",2))
		{
			memcpy(miarg[1]+0,arc[i+1],50);
		}
		if(!memcmp(arc[i],"-c",2))
		{
			memcpy(miarg[2]+0,arc[i+1],50);
		}
		if(!memcmp(arc[i],"-b",2))
		{
			memcpy(miarg[3]+0,arc[i+1],50);
		}
         }
}
